...
func TestSelection_SingleFieldAliasComposesWithObject(t *testing.T) {
	// One field + an explicit alias: the alias must show, composed with the object label
	// (matched segment) so multiple wildcard series stay distinct.
	topic := mkTopic("mqtt/VPN/solace1025/gw/queue/rdp-influx-gw", "info/num-messages-spooled")
	topic.SeriesName = "solace1025/rdp-influx-gw" // as set by the wildcard expansion
	topic.FieldAliases = map[string]string{"info/num-messages-spooled": "Current Messages"}
	topic.appendMessage(Message{Timestamp: time.Now(), Value: []byte(`{"info":{"num-messages-spooled":42}}`)})

	frame, err := topic.SeedFrame(log.DefaultLogger)
	require.NoError(t, err)
	require.Equal(t, "solace1025/rdp-influx-gw/Current Messages", frame.Fields[1].Config.DisplayNameFromDS)
}

func TestSelection_SingleFieldNoAliasIsObjectOnly(t *testing.T) {
	// One field, no alias: the object label alone (no trailing separator/field name).
	topic := mkTopic("mqtt/VPN/solace1025/gw/queue/rdp-influx-gw", "info/num-messages-spooled")
	topic.SeriesName = "solace1025/rdp-influx-gw"
	topic.appendMessage(Message{Timestamp: time.Now(), Value: []byte(`{"info":{"num-messages-spooled":42}}`)})

	frame, err := topic.SeedFrame(log.DefaultLogger)
	require.NoError(t, err)
	require.Equal(t, "solace1025/rdp-influx-gw", frame.Fields[1].Config.DisplayNameFromDS)
}
